...
```{r}
q2 <- referrals %>% select(CONTACT_NUMBER,GENDER,DISABILITY_STATUS,MILITARY_STATUS,REFERRED_AGENCY_NAME) %>% distinct()
q2 %>% select(-c(CONTACT_NUMBER)) %>% tbl_summary(by = REFERRED_AGENCY_NAME,missing = "no")
```
